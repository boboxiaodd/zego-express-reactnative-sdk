package im.zego.reactnative.faceunity;
...
abstract class IFURenderer {
     ...
     * 渲染属性
     */
    protected FUExternalInputEnum externalInputType = FUExternalInputEnum.EXTERNAL_INPUT_TYPE_VIDEO;//数据源类型
    protected FUInputTextureEnum inputTextureType = FUInputTextureEnum.FU_ADM_FLAG_EXTERNAL_OES_TEXTURE;//纹理类型
    protected FUInputBufferEnum inputBufferType = FUInputBufferEnum.FU_FORMAT_RGBA_BUFFER;//数据类型
    ...
    /**
     * 双输入接口，输入 buffer，必须在具有 GL 环境的线程调用
     *
     ...
     * @return
     */
    public abstract FURenderOutputData onDrawFrameSingleInput(byte[] img, int width, int height, boolean withBuffer);
